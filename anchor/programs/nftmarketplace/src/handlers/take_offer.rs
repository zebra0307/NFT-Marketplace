...
pub fn take_offer(context: Context<TakeOffer>) -> Result<()> {
    let (expected_mint, amount_due) = match context.accounts.offer.payment {
        PaymentKind::Token { mint, amount } => (mint, amount),
        PaymentKind::Sol { .. } => return err!(ErrorCode::UnsupportedPaymentMethod),
    };

    require!(
        context.accounts.token_mint_b.key() == expected_mint,
